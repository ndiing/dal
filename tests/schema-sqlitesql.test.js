...
const Table = require("../src/db/table.js");
...
describe("schema-sqlitesql", () => {
    test("inline constraint", () => {
        ...
        expect(result).toEqual([
            {
                query: "CREATE TABLE kategori (id_kategori INT PRIMARY KEY AUTOINCREMENT, nama_kategori VARCHAR(50) NOT NULL);",
            },
            {
    ...
    test("multiple block constraint", () => {
        ...
        expect(result).toEqual([
            ...
            {
                query: "CREATE TABLE profil_pengguna (email VARCHAR(100), nama_lengkap VARCHAR(100), UNIQUE (email), PRIMARY KEY (email), CHECK (email LIKE '%@%.%'), FOREIGN KEY (email) REFERENCES akun_induk (email_induk));",
            },
            {
                query: "CREATE INDEX IX_profil_pengguna_email ON profil_pengguna (email);",
            },
    ...
    test("multiple block named constraint", () => {
        ...
        expect(result).toEqual([
            ...
            {
                query: "CREATE TABLE profil_pengguna (email VARCHAR(100), nama_lengkap VARCHAR(100), CONSTRAINT PK_name PRIMARY KEY (email), CONSTRAINT UQ_name UNIQUE (email), CONSTRAINT CK_name CHECK (email LIKE '%@%.%'), CONSTRAINT FK_name FOREIGN KEY (email) REFERENCES akun_induk (email_induk));",
            },
            { query: "CREATE INDEX IX_name ON profil_pengguna (email);" },
        ]);
    });
    ...
    test("create table", () => {
        const result = db
            ...
            .createTable("users", (table) => {
                table.column("id").int().primaryKey();
                table.column("name").varChar(100).notNull();
            })
            .build();
        expect(result).toEqual([
            {
                query: "CREATE TABLE users (id INT PRIMARY KEY, name VARCHAR(100) NOT NULL);",
            },
        ]);
    });
    ...
    test("alter table", () => {
        ...
        expect(result).toEqual([
            {
                query: "ALTER TABLE users ADD COLUMN id INT PRIMARY KEY, ADD COLUMN name VARCHAR(100) NOT NULL;",
            },
        ]);
    });
    ...
    test("alter table", () => {
        const result = db
            .schema()
            .alterTable("users", (table) => {
                table.alterColumn("id").int().primaryKey();
                table.alterColumn("name").varChar(100).notNull();
            })
            .build();
        expect(result).toEqual([
            {
                query: "ALTER TABLE users ALTER COLUMN id TYPE INT, ALTER COLUMN name TYPE VARCHAR(100), ALTER COLUMN name SET NOT NULL;",
            },
        ]);
    });
    ...
    test("alter table", () => {
        const result = db
            .schema()
            .alterTable("users", (table) => {
                table.dropColumn("id").int().primaryKey();
                table.dropColumn("name").varChar(100).notNull();
            })
            .build();
        expect(result).toEqual([
            {
                query: "ALTER TABLE users DROP COLUMN id, DROP COLUMN name;",
            },
        ]);
    });
