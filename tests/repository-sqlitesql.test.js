...
describe("repository-sqlitesql", () => {
    beforeAll(async () => {
        ...
        await db.schema().createTable("users", (table) => {
            ...
            table.column("email").text().notNull().unique();
            table.column("age").int().notNull();
            table.column("is_active").int().default(1);
            table.column("created_at").text().default(db.raw("CURRENT_TIMESTAMP"));
            table.column("updated_at").text().default(db.raw("CURRENT_TIMESTAMP"));
