...
describe("query-tsql", () => {
    ...
    //     expect(result).toEqual({
    //         query: "MERGE INTO users WITH (HOLDLOCK) AS target USING (VALUES (@nama, @email)) AS source (nama, email) ON target.nama = source.nama AND target.email = source.email WHEN NOT MATCHED THEN INSERT (nama, email) VALUES (source.nama, source.email);",
    //         params: { nama: "Budi", email: "budi@mail.com" },
    //     });
    // });
    /**@deprecated*/
    ...
    //     expect(result).toEqual({
    //         query: "MERGE INTO users WITH (HOLDLOCK) AS target USING (VALUES (@id, @nama, @email)) AS source (id, nama, email) ON target.id = source.id AND target.nama = source.nama AND target.email = source.email WHEN MATCHED THEN UPDATE SET target.id = source.id, target.nama = source.nama, target.email = source.email WHEN NOT MATCHED THEN INSERT (id, nama, email) VALUES (source.id, source.nama, source.email);",
    //         params: { id: 1, nama: "Budi", email: "budi@mail.com" },
    //     });
    // });
    test("UPSERT (SQLite 3.24+) - Yang Bener", () => {
        const result = db
            ...
            .build();
        expect(result).toEqual({
            query: "MERGE INTO users WITH (HOLDLOCK) AS target USING (VALUES (@id, @nama, @email, @umur)) AS source (id, nama, email, umur) ON target.id = source.id WHEN MATCHED THEN UPDATE SET target.nama = source.nama, target.email = source.email, target.umur = source.umur WHEN NOT MATCHED THEN INSERT (id, nama, email, umur) VALUES (source.id, source.nama, source.email, source.umur);",
    ...
    test("mirip INSERT OR IGNORE", () => {
        const result = db
            ...
            .build();
        expect(result).toEqual({
            query: "MERGE INTO users WITH (HOLDLOCK) AS target USING (VALUES (@id, @nama, @email, @umur)) AS source (id, nama, email, umur) ON target.id = source.id WHEN NOT MATCHED THEN INSERT (id, nama, email, umur) VALUES (source.id, source.nama, source.email, source.umur);",
            params: { id: 1, nama: "Budi", email: "budi@mail.com", umur: 26 },
        });
    });
    test("Conditional update", () => {
        ...
        expect(result).toEqual({
            query: "MERGE INTO users WITH (HOLDLOCK) AS target USING (VALUES (@email, @umur)) AS source (email, umur) ON target.email = source.email WHEN MATCHED AND excluded.umur > users.umur THEN UPDATE SET target.umur = source.umur WHEN NOT MATCHED THEN INSERT (email, umur) VALUES (source.email, source.umur);",
    ...
    test("RETURNING (SQLite 3.35+)", () => {
        ...
        expect(result).toEqual({
            query: "INSERT INTO users (nama, email) OUTPUT INSERTED.id, INSERTED.nama, INSERTED.created_at VALUES (@nama, @email);",
    ...
    test("skip 20, ambil 10", () => {
        ...
        expect(result).toEqual({
            query: "SELECT * FROM users ORDER BY (SELECT NULL) OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY;",
    ...
    test("Daripada: LIMIT 10 OFFSET 100000", () => {
        ...
        expect(result).toEqual({
            query: "SELECT TOP (@limit) * FROM users WHERE id > @param ORDER BY id;",
            params: { limit: 10, param: 100000 },
        });
    });
    test("unique values", () => {
    ...
    test("UPDATE ... RETURNING", () => {
        const result = db
            ...
            .build();
        expect(result).toEqual({
            query: "UPDATE users SET umur = umur + 1 OUTPUT INSERTED.id,DELETED.id, INSERTED.nama,DELETED.nama, INSERTED.umur,DELETED.umur WHERE kota = @param;",
    ...
    test("UPDATE dengan LIMIT (Perlu Compile Option)", () => {
        ...
        expect(result).toEqual({
            query: "UPDATE TOP (@limit) users SET status = @status WHERE umur < @param ORDER BY id;",
            params: { limit: 100, status: "inactive", param: 18 },
        });
    });
    test("Basic Delete", () => {
    ...
    test("DELETE ... RETURNING", () => {
        ...
        expect(result).toEqual({
            query: "DELETE FROM users OUTPUT DELETED.id, DELETED.nama WHERE umur > @param;",
            params: { param: 100 },
        });
    });
    test("INNER JOIN", () => {
